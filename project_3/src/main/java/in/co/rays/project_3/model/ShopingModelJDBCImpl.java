...
public class ShopingModelJDBCImpl implements ShopingModelInt{
	public long nextPK() throws DatabaseException {
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement("select max(id) from ST_Shoping");
	...
	public long add(UserDTO dto) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			PreparedStatement ps = con
					.prepareStatement("insert into ST_Shoping values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			ps.setLong(1, pk);
	...
	public void delete(UserDTO dto) throws ApplicationException {
		...
		try {
			con = JDBCDataSource.getConnection();
			con.setAutoCommit(false);
			PreparedStatement ps = con.prepareStatement("delete from ST_Shoping where id=?");
	...
	public void update(UserDTO dto) throws ApplicationException, DuplicateRecordException {
		...
		ShopingDTO dtoExist = findByLogin(dto.getLogin());
		...
		try {
			...
			ps = con.prepareStatement(
					"update ST_Shoping set FIRST_NAME=?,LAST_NAME=?,LOGIN=?,PASSWORD=?,CONFIRMPASSWORD=?,DOB=?,MOBILE_NO=?,ROLE_ID=?,UNSUCCESSFUL_LOGIN=?,GENDER=?,LAST_LOGIN=?,REGISTERED_IP=?,LAST_LOGIN_IP=?,CREATED_BY=?,MODIFIED_BY=?,CREATED_DATETIME=?,MODIFIED_DATETIME=? WHERE ID=?");
