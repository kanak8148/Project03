...
//		}
	...
	public List search(CustomerDTO dto, int pageNo, int pageSize) throws ApplicationException {
		...
		try {
			...
			if (dto.getId() != null && dto.getId() > 0) {
			  ...
			  if (dto.getClintName() != null && dto.getClintName().length() > 0) {
			  criteria.add(Restrictions.like("clintName", dto.getClintName() + "%")); } if
			  (dto.getLocation() != null && dto.getLocation().length() > 0) {
			  criteria.add(Restrictions.like("location", dto.getLocation() + "%")); }
			  if (dto.getContactNumber() != null && dto.getContactNumber() > 0) {
	...
	public CustomerDTO fingByName(String name) throws ApplicationException {
		Session session = null;
		CustomerDTO dto = null;
		try {
			session = HibDataSource.getSession();
